// Problem:      P2763 试题库问题
// Contest:      Luogu
// URL:          https://www.luogu.com.cn/problem/P2763
// Memory Limit: 125 MB
// Time Limit:   1000 ms
// Created Time: 2023-07-03 18:19:01 

#include <iostream>

#include <queue>
#include <vector>
#include <cstring>

#define i64 long long
#define endl '\n'
#define pb push_back
#define qwq puts("fzy qwq ~")

using namespace std;

const int N = 4e3 + 10;
const int M = 2e6 + 10;
const int inf = 0x7fffffff;

struct Arc
{
	int u, v, f, nxt; Arc() { }
	Arc(int u, int v, int f, int nxt):
		u(u), v(v), f(f), nxt(nxt) { }
} e[M << 2];
int h[N], tot = 1;
inline void addarc(int u, int v, int f)
{
	e[++tot] = Arc(u, v, f, h[u]), h[u] = tot;
	e[++tot] = Arc(v, u, 0, h[v]), h[v] = tot;
}

int n, m, k;
int r, p, x, cnt, act;
int maxflow;
int d[N], cur[N];
int faq[N];
vector<int> res[N];

inline bool bfs(int S, int T)
{
	queue<int> q; q.push(S);
	
	memset(d, -1, sizeof d);
	d[S] = 0, cur[S] = h[S];
	
	while (!q.empty())
	{
		int u = q.front(); q.pop();
		for (int i = h[u]; i; i = e[i].nxt)
		{
			int v = e[i].v;
			if (!~d[v] && e[i].f)
			{
				d[v] = d[u] + 1;
				cur[v] = h[v];
				if (v == T) return 1;
				q.push(v);
			}
		}
	}
	return 0;
}
inline int find(int u, int lmt, int T)
{
	if (u == T) return lmt;
	int flow = 0; 
	for (int i = cur[u]; i && flow < lmt; i = e[i].nxt)
	{
		cur[u] = i; int v = e[i].v;
		if (d[v] == d[u] + 1 && e[i].f)
		{
			int t = find(v, min(e[i].f, lmt - flow), T);
		   	if (!t) d[v] = -1;
		   	e[i].f -= t, e[i ^ 1].f += t; flow += t;
		}
	}
	return flow;
}
inline int dinic(int S, int T)
{
	int res = 0, flow;
	while (bfs(S, T))
		while ( flow = find(S, inf, T) )
			res += flow;
	return res;
}

int main()
{
	ios::sync_with_stdio(0);
	cin.tie(0), cout.tie(0);
	
	cin >> k >> n;
	for (int i = 1; i <= k; ++i)
	{
		cin >> r; m += r;
		addarc(i, 2021, r);
	}
	for (int i = 1; i <= n; ++i)
	{
		cin >> p;
		addarc(2020, k + i, 1);
		for (int j = 1; j <= p; ++j)
			cin >> x, faq[++cnt] = tot + 1, addarc(k + i, x, 1);
	}
	maxflow = dinic(2020, 2021);
	if (maxflow < m) { cout << "No Solution!" << endl; exit(0); }
	
	for (int i = 1; i <= cnt; ++i)
		if (e[faq[i]].f == 0)
			res[e[faq[i]].v].pb(e[faq[i]].u - k);
	
	// for (int i = 1; i <= cnt; ++i)
		// cerr << e[faq[i]].u - k << ' ' << e[faq[i]].v << ' ' << e[faq[i]].f << endl;
	
	for (int i = 1; i <= k; ++i)
	{
		cout << i << ": ";
		for (int j : res[i])
			cout << j << ' ';
		cout << endl;
	}
	return 0;
}